export default class {
  ...
  init () {
    ...
    addProxyBtn.addEventListener('click', () => {
      createProxy(['Not selected', 'xxx.xxx.xxx.xxx', 'xxx.xxx.xxx.xxx', '', '', '',  'xxx.xxx.xxx.xxx'])
    })
    ...
    const checkIP = (inputField, type, newData) => {
        let newSTR;
        let pos = inputField.selectionStart;
        const dataArr = []

        if(inputField.value.length === 0){
          newSTR = 'xxx.xxx.xxx.xxx'.split(".");
        }else {
          newSTR = inputField.value.split(".");
        }

        console.log('POS ' + pos);

        if(type === 'del'){
          for(let row of newSTR){
            if(row === '') {row = 0}
            if(parseInt(row) >= 255) {row = 255}
          }
            newSTR = newSTR.join('.')
        }
        if(type === 'add'){
              ...
              x.addEventListener('keyup', (e) => {
                ...
                if ((e.which >= 48 && e.which <= 57)  // цифры
                        ...
                        || (e.which >= 37 && e.which <= 40)){
                          checkIP(x, 'add', parseInt(e.key))
                }


                if((e.which >= 37 && e.which <= 40) || e.which == 46 || e.which == 8 ) {
                  //console.log(e.which);
                  checkIP(x, 'del')
                  e.stopPropagation();
                  e.preventDefault()
