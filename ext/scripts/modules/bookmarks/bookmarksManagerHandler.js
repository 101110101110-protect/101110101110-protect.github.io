import BookmarksSingleDataViewComponent from './bookmarksSingleDataViewComponent.js'
...
export default class {
  ...
  initDefaultData() {
    ...
    const mainMW = document.querySelectorAll('.mw__bookmarks')[0];
    let mwPrivate = mainMW.querySelectorAll('.bookmarksTab')[0]



    this._STATE_bookmarks = {
      sortByName: {
        isActive: false,
        isReverse: false
      },
    ...
    this._VIEW_bookmarks = {
      table: mwPrivate.querySelectorAll('.static_table .tbody')[0],
      tableItemComponent: BookmarksSingleDataViewComponent,
    ...
    this._STORE_bookmarks = [
      {
        url : 'https://ea.com/games/fifa2019',
      },
      {
        url : 'https://reddit.com/thread/12381029484301',
      },
    ]
  ...
  initBookmarks() {
    ...
    const rootProps = {
      store: this._STORE_bookmarks,
      state: this._STATE_bookmarks,
      view: this._VIEW_bookmarks
    }
  ...
  getRowHandlers(props, tableName, id) { // timestamp -- uniq ID
    ...
    const checkboxHandler = (checkboxVal) => {
        if(checkboxVal){
          this._STATE_bookmarks.checkedIDs.push(id)
        }else {
          const index = this._STATE_bookmarks.checkedIDs.indexOf(id);
          if (index > -1) {
            this._STATE_bookmarks.checkedIDs.splice(index, 1);
          }
        }
    }
