  ...
  init () {
    ...
    const initHeightForRow = (rowId, rowsInside) => {
      if(rowsInside !== 0 || rowsInside !== 1){
        let row = mwWrapper.querySelectorAll('[data-id="' + rowId + '"]')
        for (const rowElem of row) {
        ...
        for (const column of columns) {
          if (colCounter === 0) {
            ...
            if (searchInput.value.length !== 0) {
              ...
              colContent = colData.toString().replace(/(<span class='marker'>|<\/span>)/igm, "");
            }
            ...
            if (colCounter === 5) {
              ...
              for(let mimeItem of mimeData) {
                ...
                let text = document.createTextNode(mimeItem.type);
                mimeWrapper.appendChild(getMimeNav())
              ...
              for(let mimeItem of mimeData) {
                const mimeWrapper = document.createElement('DIV')
                mimeWrapper.innerHTML = mimeItem.description
                wrapper.appendChild(mimeWrapper)
              }
              divWrapper.appendChild(wrapper)
              ...
              for(let mimeItem of mimeData) {
                const mimeWrapper = document.createElement('DIV')
                mimeWrapper.innerHTML = mimeItem.suffix
                wrapper.appendChild(mimeWrapper)
              }
              divWrapper.appendChild(wrapper)
            }else {
